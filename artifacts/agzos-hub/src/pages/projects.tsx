...
import { Plus, ListFilter, Calendar, Clock } from "lucide-react";
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Track agency projects, tasks, and deadlines.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard title="Total Projects" value={summary?.total} loading={summaryLoading} />
        <StatCard title="Active" value={summary?.byStatus?.active} loading={summaryLoading} className="text-primary" />
        <StatCard title="Completed (Month)" value={summary?.completedThisMonth} loading={summaryLoading} className="text-emerald-500" />
        <StatCard title="Overdue" value={summary?.overdue} loading={summaryLoading} className="text-destructive" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {isLoading ? (
          ...
          projects?.map((project) => (
            <Card key={project.id} className="border-border/50 bg-card/50 backdrop-blur-xl hover:border-primary/50 transition-colors group cursor-pointer overflow-hidden">
              ...
              <CardContent className="p-5 flex flex-col md:flex-row gap-6 md:items-center">
                <div className="flex-1 space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1 line-clamp-1">{project.description || 'No description provided'}</p>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-muted-foreground">
                    ...
                    {project.dueDate && (
                      <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> {new Date(project.dueDate).toLocaleDateString()}</span>
                    )}
                  </div>
                </div>
                ...
                <div className="w-full md:w-64 shrink-0 space-y-2">
                  <div className="flex justify-between items-end text-sm">
                    <span className="font-medium text-foreground capitalize">{formatStatus(project.status)}</span>
